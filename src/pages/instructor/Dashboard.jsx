  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="card bg-gradient-to-r from-[#4285F4] to-blue-500 text-white border-0">
        <p className="text-blue-100 text-sm font-medium">{t('dashboard', 'welcome')}</p>
        <h1 className="text-2xl font-bold mt-1">{user?.fullName || user?.email}</h1>
        <p className="text-blue-100 text-sm mt-1">{t('dashboard', 'instructor')} · {new Date().toLocaleDateString(settings.language === 'en' ? 'en-US' : 'es-CO', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-900">{t('dashboard', 'myGroups')}</h2>
            ...
            <div className="space-y-2">
              {fichas.slice(0, 4).map(f => (
                <div key={f.id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                  <div>
                    <p className="font-semibold text-sm text-gray-800">{t('dashboard', 'groupPrefix')} {f.numero}</p>
                    <p className="text-xs text-gray-400">{f.nivel} · {f.jornada}</p>
                  </div>
                  <div className="text-right">
                    <span className="badge badge-info">{f.aprendices?.length || 0} {t('dashboard', 'studentsLower')}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Acciones rápidas */}
        <div className="card">
          <h2 className="font-bold text-gray-900 mb-4">{t('dashboard', 'quickActions')}</h2>
          <div className="grid grid-cols-2 gap-3">
            {[
              { to: '/instructor/fichas',     icon: Plus,     label: t('dashboard', 'actionNewGroup'),      color: 'bg-blue-50 text-[#4285F4]' },
              { to: '/instructor/asistencia', icon: Clock,    label: t('dashboard', 'actionStartSession'),   color: 'bg-green-50 text-[#34A853]' },
              { to: '/instructor/excusas',    icon: FileText, label: t('dashboard', 'actionViewExcuses'),      color: pendientes > 0 ? 'bg-yellow-50 text-[#FBBC05]' : 'bg-gray-50 text-gray-500' },
              { to: '/instructor/horario',    icon: Calendar, label: t('dashboard', 'actionSchedule'),       color: 'bg-purple-50 text-purple-500' },
              ...
              <Link key={to} to={to}
                ...
                {label === t('dashboard', 'actionViewExcuses') && pendientes > 0 && (
                  <span className="badge badge-pending text-xs">{pendientes} {t('dashboard', 'pendingLower')}</span>
                )}
              </Link>
            ))}
          </div>
      ...
      {materias.length > 0 && (
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-gray-900">{t('dashboard', 'mySubjects')}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {materias.slice(0, 6).map(m => (
              <div key={m.id} className="p-3 rounded-xl bg-gray-50 border border-gray-100">
                ...
                <p className="text-xs text-gray-400 mt-0.5">{t('dashboard', 'groupPrefix')} {m.ficha?.numero}</p>
